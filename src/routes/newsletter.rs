...
fn basic_authentication(headers: &HeaderMap) -> Result<Credentials, anyhow::Error> {
    ...
    let base64_encoded_segment = header_value.strip_prefix("Basic ")
    ...
    let username = credentials
        .next()
        .ok_or_else(|| {
            anyhow::anyhow!("A username must be provided for 'Basic' auth.")
        })?
        .to_string();
    let password = credentials
        .next()
        .ok_or_else(|| {
            anyhow::anyhow!("A password must be provided for 'Basic' auth.")
        })?
        .to_string();
